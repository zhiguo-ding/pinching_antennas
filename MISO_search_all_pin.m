...
parfor isnr = 1 : length(snrvec)
    ...
    for ict = 1 : ct
        ...
        stepsize = lambda/40;
        pinvec = [-10*lambda: stepsize : 10*lambda]; %location range
        max_value = 0;
        sinr = zeros(length(pinvec),length(pinvec));
        for i = 1 : length(pinvec)
            for j = 1 : length(pinvec)
                %pinching antennas' locations
                pin1 = [loc(1,1)+pinvec(i) D/3]; %[pinvec(i)+loc(1,1) D/3]; 
                pin2 = [loc(2,1)+pinvec(j) -D/3]; 
        ...
        %MRC
        sinr1_mrc = h1'*h1/(abs(h1'*h2)^2/(h1'*h1)+1/snr);
        sinr2_mrc = h2'*h2/(abs(h2'*h1)^2/(h2'*h2)+1/snr);
